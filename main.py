...
if __name__ == "__main__":
    ...
    logging.info(f"主邮箱: {config.email_config.email}")
    logging.info(f"接收邮箱: {config.receive_email}")
    logging.info(f"问财查询条件: {config.wencai_query}")
    logging.info(f"execution_time: {time_str}")
    # schedule_job()
    while True:
